...
func TestBoard_MakeCellAndSurroundingCellsOpened(t *testing.T) {
	t.Parallel()

	boardCfg := board.Config{NumRows: 4, NumCols: 4}
	...
	expectedStateBefore := [][]board.CellValue{
		{"?", "?", "?", "?"},
		{"?", "?", "?", "?"},
		{"?", "?", "?", "?"},
		{"?", "?", "?", "?"},
	...
	currentState := gameBoard.State()
	testhelpers.EqualBoardStates(t, expectedStateBefore, currentState)

	gameBoard.MakeCellAndSurroundingCellsOpened(0, 0)

	expectedStateAfter := [][]board.CellValue{
		{"0", "0", "0", "0"},
		{"0", "0", "1", "1"},
		{"2", "2", "2", "?"},
		{"?", "?", "?", "?"},
	...
	currentState = gameBoard.State()
	testhelpers.EqualBoardStates(t, expectedStateAfter, currentState)
}
